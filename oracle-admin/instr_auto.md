...
- _ **Bold and Italicized** _ text is used to highlight text that must be typed in the UI, rather than selected (e.g. Enter _ **devdb** __)_.
...
2. Login to the Delphix Dynamic Data Platform using the username _ **admin** _ and password _ **delphix** _
3. Click **Log In**
...
4. Login to **Delphix Setup** using the username _ **sysadmin** _ and password _ **sysadmin** _
5. Click **Log In**
...
1. Log into the Delphix Data Platform UI as the _ **admin** _ user and the password that was set during the Engine setup.
...
1. For Environment Settings, provide the following information
  ...
  2. Host Address: _ **10.0.x.20** _ (&#39;x&#39; will be your **Student Number** )
  3. NFS Addresses: _ **10.0.x.20** _ (&#39;x&#39; will be your **Student Number** )
  4. Leave the **DSP** fields blank
  5. Set **Login Type** to **Username and Password**
  6. OS Username: _ **delphix** _
  7. OS Password: _ **Delphix** _
...
3. Under the **dSource Configuration** tab we will provide a user-friendly name for the dSource and create a new Dataset Group to place it in.
  1. Enter _ **orcl** _ for the **dSource Name**
  2. Click on the **Add Dataset Group** link and enter _ **Source DB** _ in the **Name** field.
...
1. On the **Environment Settings** tab, enter the following details
  1. Environment Name: _ **Target** _
  2. Host Address: _ **10.0.x.30** _ (&#39;x&#39; will be your **Student Number** )
  3. NFS Addresses: _ **10.0.x.30** _ (&#39;x&#39; will be your **Student Number** )
  4. Leave the **DSP** fields blank
  5. OS Username: _ **delphix** _
  6. OS Password: _ **delphix** _
  7. Toolkit Path: _ **/u01/app/toolkit** _ (The Toolkit Path must be entered prior to clicking Validate)
  8. Click **Validate**
2. Click **Next**
...
1. Enter the details below on the **Target Configuration** tab , we will provide a user-friendly name for the VDB as well as assign it to a new group.
  1. Click **Add Dataset Group**
    1. Type _ **DB Targets** _ for the **Group Name**
    2. Click **Add**
  2. Verify that **DB Targets** is selected as the **Target Group**
  3. **Mount Base** : Type _ **/mnt/provision** _
  4. **Oracle Database Name** : Type _ **devdb** _
  5. **VDB Name** : Type _ **devdb** _
  6. **Oracle Database Unique Name** : Type _ **devdb** _
  7. **Oracle SID** : _ **devdb** _
  8. Click **Next**
...
1. Provide the following details:
  1. Policy Name: _ **Long Term** _
  2. Keep Logs for: _ **30** _ **days**
  3. Keep Snapshots for: _ **30** _ **days**
2. Click the _ **Show advanced** _ _link_
...
3. Provide the _Name_: _ **Create APPUSER** _
...
4. In the row that is now highlighted, enter the following information:
  1. Name: _ **sga** __**\_**__ **max\_size** _
  2. Value: _ **1G** _
...
6. On the **Target Configuration** tab, enter the following details:
  ...
  3. **Database Name** : _ **qadb** _
  4. **Database Unique Name** : _ **qadb** _
  5. **SID** : _ **qadb** _
...
1. **Enter a Name** _ **QA APPUSER** _ for the **Hook Operation**
...
2. On the Add Dataset Group pop up, add a new group with the name _ **Ora12C Sources** _.
3. Click **Add**.
...
3. On the Add Dataset Group Wizard create a new group with the name _ **Ora12C Targets** _.
4. Click **Add**
...
2. Add a **Replication Profile** called DR Replica
  ...
  2. Enter a **Replica Profile Name** : _ **DR Replica** _
  ...
  4. Enter the User Name: _ **admin** _
  5. Enter the Password: _ **delphix** _
...
1. Check the results on your target Delphix Data Platform
  1. In your lab server browser, enter _ **the IP address** _ you used for the Target Engine in your replica profile. For example, if you are Student 1, your Delphix Data Platform is at 10.0.1.10, and your target would have been 10.0.2.10.
  2. Log in as user _ **admin** _ with the password _ **delphix** _
